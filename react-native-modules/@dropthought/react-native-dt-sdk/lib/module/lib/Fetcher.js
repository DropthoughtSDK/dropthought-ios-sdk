...
export const isNoInternetError = error => error.message === 'Network request failed';

const isTokenValid = jwtToken => {
  let jwtPayload; // decode token

  try {
    jwtPayload = jwtDecode(jwtToken); // check expiration date
...
export class Fetcher {
  ...
  async fetchWithTimeout(url, requestConfig) {
    ...
    const params = Object.keys(requestConfig.params || {}).filter(key => !isNil(requestConfig.params[key])).map(key => `${key}=${encodeURIComponent(requestConfig.params[key])}`).join('&'); // compose fetch full URL
  ...
  async request(url, requestConfig) {
    ...
    if (requestConfig.cache) {
      await this.saveCache(url, pick(['data', 'status', 'statusText', 'headers'], response));
    }
