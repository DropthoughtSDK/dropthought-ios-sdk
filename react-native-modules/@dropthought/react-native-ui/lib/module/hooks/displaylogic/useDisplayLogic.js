...
const useDisplayLogic = ({
  ...
  const questionValidation = (condition, displayLogic, feedback) => {
    ...
    if (conditaionList.length <= 3) {
      ...
      if (operator === 'answ') {
        ...
        if (feedback) {
          answerExist = feedback.answers !== undefined && feedback.answers.length > 0 && !isEmpty(feedback.answers[0]) && feedback.answers[0] !== '-1';
      ...
      if (options !== undefined) {
        ...
        if (operator === 'answ') {
          ...
          if (Array.isArray(answer)) {
            allowToDisplay = !isEmpty(answer) && answer.every(value => value !== -1);
          } else {
            allowToDisplay = answer !== undefined && !isEmpty(answer) && answer !== -1;
          }
        } else if (operator === 'nasr') {
  ...
  const [displayDictionary, setDisplayDictionary, displayDictionaryRef] = useStateRef(() => {
    const result = {};
