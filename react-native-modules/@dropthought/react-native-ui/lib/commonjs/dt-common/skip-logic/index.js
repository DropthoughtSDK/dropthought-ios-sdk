...
Object.keys(_IfcRule).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (Object.prototype.hasOwnProperty.call(_exportNames, key)) return;
  if (key in exports && exports[key] === _IfcRule[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _IfcRule[key];
...
const handleSplit = (condition, type) => {
  ...
  let conditionArrMax; // if the condition include '.' text, we need to merge the condition
  ...
  if (questionId === conditionArr[0]) {
    if (conditionArr.length === 4) {
        ...
        case 'matrixRating':
          {
            ...
            selectedAnswer = parseInt(textOrIndexArr, 10); // e.g. conditionArr[3] => '1', '1-2', '1,2'
      ...
      switch (skiplogicType) {
        ...
        case 'nasw':
          {
            const textOrIndexArr = `${feedback.textOrIndexArr[0]}`; // e.g. is Not Answered with "1,2"
