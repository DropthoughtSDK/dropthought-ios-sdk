...
const ClassicStatementQuestion = ({ question, onFeedback }: Props) => {
  const { questionTitle, statementProperty, questionId } = question;
  ...
  const imageComponent = () => {
    if (addImage) {
      if (imageLoadError) {
        ...
        return (
          <View
            accessibilityLabel={`test:id/statement_image_${getImageExtension(
              logo
            )}`}
...
export default React.memo(ClassicStatementQuestion);
